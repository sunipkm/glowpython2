...
!     edel    energy bin width in ev
!     nbins  number of energy bins (dimension of ener, edel, and phi)
...
!     phi    hemispherical flux in cm-2 s-1 ev-1
  ...
  subroutine maxt (eflux,ezer,ener,edel,nbins,itail,fmono,emono,phi)

    implicit none

    integer,intent(in) :: nbins, itail
    real,intent(in) :: eflux, ezer, ener(nbins), edel(nbins), fmono, emono
    ...
    do k=1,nbins
      ...
      te = te + phi(k) * edel(k) * ener(k) * 1.6022e-12
    enddo
    ...
    if (fmono > 0.) then
      do k=1,nbins
        if (emono > ener(k)-edel(k)/2. .and. emono < ener(k)+edel(k)/2.) &
          phi(k)=phi(k)+fmono/(1.6022e-12*edel(k)*ener(k))
      enddo
    endif
