...
!
    DO I=1,JMAX
      GH = (RE+ZZ(I)) * SIN(SZA)
      IF (SZA .LT. 1.6 .OR. GH .GT. RE) THEN ! sza < pi/2 or sza > pi/2 but sun not blocked by earth at high altitudes
        G(1,I) = 0.041
        G(2,I) = 0.013
      ENDIF
    ENDDO
...
!
      DO I=1,JMAX
            T1(I) = (16.*ZTN(I)+28.*ZTI(I)) / (16.+28.) / 300. ! O and N2
            T2(I) = (16.*ZTN(I)+32.*ZTI(I)) / (16.+32.) / 300. ! O and O2
            T3(I) = (28.*ZTN(I)+16.*ZTI(I)) / (28.+16.) / 300. ! N2 and O+
            T4(I) = (28.*ZTN(I)+32.*ZTI(I)) / (28.+32.) / 300. ! N2 and O2+
            T5(I) = (16.*ZTN(I)+30.*ZTI(I)) / (16.+30.) / 300. ! O and NO+
            ...
            IF (ZTE(I) .GE. 1200.) THEN
                  KZ(24,I) = 1.6E-07 * (300./ZTE(I)) ** 0.55
            ELSE
