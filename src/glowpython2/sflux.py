...
from typing import Sequence, Tuple, SupportsFloat as Numeric
import numpy as np
...
class Sflux:
    def __init__(
        self,
        kind: FluxSource | Tuple[Sequence[float], Sequence[float], Sequence[float]],
    ...
    def update(self, f107: Numeric, f107a: Numeric, xuvfac: int = 3) -> None:
        ...
        if self._docalc:
            if self._data.shape[1] == 5:  # Hinteregger
                ...
                raise ValueError(f"Unexpected data shape: {self._data.shape}. Expected 4 or 5 columns.")
            cg.sflux = sflux
        else:
